...
public class OptimizableItem {
    public final String name;
    public final List<Stat> stats = new ArrayList<Stat>();
    
    public OptimizableItem(final String name, final Stat ... stats) {
        this.name = name;
        for(final Stat stat : stats) {
            this.stats.add(stat);
        }
    }    
    public OptimizableItem(final String name, final List<Stat> stats) {
        this.name = name;
        for(final Stat stat : stats) {
            this.stats.add(stat);
        }
    ...
    public OptimizableItem bestReforge(final ScalingFactors scalingFactors) {
        final List<OptimizableItem> items = new ArrayList<OptimizableItem>();
        OptimizableItem best = this;
        items.add(this);
        for(final Stat stat : stats) {
        ...
        for(OptimizableItem candidate : items) {
    ...
    public OptimizableItem reforge(Class<? extends Stat> reforgeFrom, Class<? extends Stat> reforgeTo) throws Exception {
        ...
        return new OptimizableItem(name + " " + reforge, newStats);
    }
